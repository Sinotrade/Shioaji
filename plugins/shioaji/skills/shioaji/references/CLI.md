...
| `SJ_AGENT_HARNESS_BROKER_SOCKET` | Trusted Unix approval broker socket; preferred when both broker transports are present |
| `SJ_AGENT_HARNESS_BROKER_URL` | Trusted loopback HTTP broker URL; must be exactly `http://localhost-or-loopback/v1/exchange` |
| `SJ_AGENT_HARNESS_BROKER_TOKEN` | Runtime-scoped HTTP broker bearer credential; set together with the broker URL |
...
| `SJ_HTTP_CORS_ORIGINS` | Comma-separated browser origins allowed by CORS (defaults to Tauri and localhost dev origins) |
