...
print(
    stock_info.margin_shortable,
    stock_info.sbl_shortable,
    stock_info.below_ref_shortable,
)
```
...
### Stock short-sale eligibility 股票券賣資格

`StockInfo` exposes three required booleans from the official TWSE/TPEx daily
lists. They are independent fields because margin short selling and securities
borrowing/lending use different channels:

| Field | 中文 | Meaning when `True` |
|-------|------|---------------------|
| `margin_shortable` | 可融券賣出 | The security is eligible for margin short selling. |
| `sbl_shortable` | 可借券賣出 | The security is eligible for securities borrowing/lending short selling. |
| `below_ref_shortable` | 可低於平盤參考價融券／借券賣出 | The exchange's below-reference price restriction is not active for this security. |

Combine the channel eligibility with the price-rule flag when the order price
is below the reference price:

```python
can_margin_short = info.margin_shortable
can_sbl_short = info.sbl_shortable
can_margin_short_below_reference = (
    info.margin_shortable and info.below_ref_shortable
)
can_sbl_short_below_reference = (
    info.sbl_shortable and info.below_ref_shortable
)
```

三個欄位是證券層級的交易所規則，不代表帳戶已取得券賣權限、一定有券源或
額度，也不保證委託一定接受。`below_ref_shortable` 不能單獨當成可融券或可
借券賣出的訊號。`TSE` 取證交所名單、`OTC` 取櫃買中心名單；`OES` 與不在
相符官方名單內的商品三欄皆為 `False`。

These fields may change with the daily Contract V2 STK update. Python handles
the update event and lazy cache refresh internally. HTTP/CLI consumers that
receive a Contract event should query the relevant Stock Info again.
